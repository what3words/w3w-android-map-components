...
internal class W3WMapManager(
    ...
    /** Select [W3WAddress.square] that contains coordinates [lat] latitude and [lng] longitude. Only one selected square is allowed at a time. This method will call [W3WTextDataSource.convertTo3wa] and, if successful, will save [W3WAddress] in [selectedAddress].
     *
    ...
    /** Remove marker at [W3WAddress.square] that contains coordinates [lat] latitude and [lng] longitude from [listOfVisibleAddresses].
     *
     * @param lat latitude coordinates within [W3WAddress.square] to be removed from the map.
     * @param lng longitude coordinates within [W3WAddress.square] to be removed from the map.
     */
    ...
    /** Finds a [W3WAddressWithStyle] inside [listOfVisibleAddresses] by checking if [latitude] and [longitude] are inside of any [W3WAddress.square].
     *
    ...
    /**
     * Select an existing [W3WAddress.square] in [listOfVisibleAddresses] where the [W3WAddress.square] contains [latitude] and [longitude].
     */
    fun selectExistingMarker(latitude: Double, longitude: Double) {
        selectedAddress = squareContains(latitude, longitude)?.address
        main(dispatchers) {
            w3WMapWrapper.updateMap()
        }
    }
