...
//endregion
    ...
    private val zoomedOutMarkerListener = GoogleMap.OnMarkerClickListener { p0 ->
        w3WMapManager.selectExistingMarker(W3WCoordinates(p0.position.latitude, p0.position.longitude))
        if(onMarkerClickedCallback != null && w3WMapManager.selectedAddress != null) {
        ...
        w3WMapManager.listOfVisibleAddresses.filter { addressWithStyle ->
            if (lastScaledBounds != null && addressWithStyle.address.center != null) {
                lastScaledBounds!!.contains(
                    LatLng(
                        addressWithStyle.address.center!!.lat,
                        addressWithStyle.address.center!!.lng
                    )
                )
            } else true
        }.forEach {
